...
> 该基类可用于控制GPIO高低电平, 从而控制外设功能开关, 重复开关等, 如LED灯, Buzzer蜂鸣器等。
...
> GPIO当前电平状态, 1高电平(LED亮/Buzzer开), 0低电平(LED灭/Buzzer关)
...
> 添加控制当前LED的GPIO, 由于可能有多个GPIO控制一个颜色灯的情况, 所以以该方式实现, 如: 黄色灯由红色和绿色同时点亮进行展示则需添加两个GPIO。
...
> 启动GPIO控制模块周期性切换高低电平(LED闪烁/Buzzer周期性开关)。
...
|参数|类型|说明|
|:---|---|---|
|on_period|int|高电平持续时间(LED亮/Buzzer开)|
|off_period|int|低电平持续时间(LED灭/Buzzer关)|
...
> 停止GPIO控制模块周期性切换高低电平(LED闪烁/Buzzer周期性开关)。
